...
const start = (pakforlay = new Client()) => {
    ...
    console.log(color(figlet.textSync('----------------', { horizontalLayout: 'default' })))
    console.log(color('[DEV]'), color('zedpopo', 'yellow'))
	...
	// kondisi ketika batas member group belum tercapai, ubah di file settings/setting.json
        ...
        await pakforlay.simulateTyping(chat.id, true).then(async () => {
          await pakforlay.sendText(chat.id, `Hai minna~, Im Popox BOT. To find out the commands on this bot type ${prefix}menu`)
        })
	    }
	}
    })
    ...
    pakforlay.onMessage(async (message) => {
        pakforlay.getAmountOfLoadedMessages() // menghapus pesan cache jika sudah 3000 pesan.
            .then((msg) => {
                if (msg >= 3000) {
                    console.log('[Popox]', color(`Loaded Message Reach ${msg}, cuting message cache...`, 'yellow'))
